<div>
    <form class="position-relative" wire:submit.prevent="submitSearch">
        <div class="search-container">
            <div class="search-bar">
                <select class="my-select border-0" wire:model.defer="searchType" data-container="body">
                    <option value="product">Product</option>
                    <option value="buylead">BuyLead</option>
                </select>
                <input type="text" wire:model.defer="searchTerm" class="search-input1" placeholder="Search..." />
                <button class="search-icon border-0" type="submit" title="search">
                    <svg xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 0 24 24" width="24">
                        <path d="M0 0h24v24H0z" fill="none"></path>
                        <path
                            d="M15.5 14h-.79l-.28-.27C15.41 12.59 16 11.11 16 9.5 16 5.91 13.09 3 9.5 3S3 5.91 3 9.5 5.91 16 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zM9.5 14C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z">
                        </path>
                    </svg>
                </button>
            </div>
        </div>
    </form>
</div>
