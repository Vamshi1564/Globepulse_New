                ...
                <div class="card">
                    <div class="card-body" x-data
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-4">
                            <h2 class="fw-bold text-primary">Refund Policy</h2>
                        </div>

                        <form id="formAccountSettings" wire:submit.prevent="AddRefund">
                            <div class="mb-3"> 
                    ...
                    <script>
                        ...
                        window.addEventListener('populateEditor', event => {
                            document.getElementById('descriptionEditor').innerHTML = event.detail;
                            document.getElementById('description').value = event.detail;
                        });
                    </script>
                                ...
                                <table class="table table-striped align-middle text-center">
                                    ...
                                    <tbody>
                                        @foreach ($refunds as $item)
                                            <tr @if($isEdit && $refundId == $item['id']) style="background-color: #e7fda9;" @endif>
                                                <td class="fw-bold">{{ $loop->iteration }}</td>
                                                <td class="text-start">{!! $item['description'] !!}</td>
                                                <td>
                                                    <button class="btn btn-sm btn-warning"
                                                        wire:click="editRefund({{ $item['id'] }})">
                                                        <i class="fa fa-edit me-1"></i>Edit
                                                    </button>
                                                    <button class="btn btn-sm btn-danger"
                                                        wire:click="DeleteRefund({{ $item['id'] }})">
