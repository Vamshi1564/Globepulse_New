                ...
                <div class="card">
                    <div class="card-body" x-data
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-4">
                            <h2 class="fw-bold text-primary">Terms & Conditions</h2>
                        </div>

                        <form id="formAccountSettings" wire:submit.prevent="AddTerms">
                            <div class="mb-3"> 
                                ...
                                <table class="table table-striped align-middle text-center">
                                    ...
                                    <tbody>
                                        @foreach ($terms as $item)
                                            <tr @if($isEdit && $termsId == $item['id']) style="background-color: #e7fda9;" @endif>
                                                <td class="fw-bold">{{ $loop->iteration }}</td>
                                                <td class="text-start">{!! $item['description'] !!}</td>
                                                <td>
                                                    <button class="btn btn-sm btn-warning"
                                                        wire:click="editTerms({{ $item['id'] }})">
                                                        <i class="fa fa-edit me-1"></i>Edit
                                                    </button>
                                                    <button class="btn btn-sm btn-danger"
                                                        wire:click="DeleteTerms({{ $item['id'] }})">
