...
<button type="button"
    ...
    title="{{ 
        ($planLimitBlocked ?? false) && !($isEditMode ?? false) 
            ? 'Upgrade plan to add products' 
            : (!empty($banError) 
                ? 'This product is banned' 
                : ($isEditMode ? 'Save changes' : 'Submit for admin review')
            ) 
    }}"

    {{ 
        (($planLimitBlocked ?? false) && !($isEditMode ?? false)) || !empty($banError) 
            ? 'disabled' 
            : '' 
    }}
